...
.use(footnote_plugin)`` 로 두 확장을 모두 활성화한다.
"""
...
def parse_markdown(md_path: str | Path) -> list[Block]:
    ...
    md = MarkdownIt("commonmark").enable("table").use(footnote_plugin)
    ...
    while i < n:
        ...
        if tok.type == "paragraph_open":
            ...
            if figure is not None and blockquote_depth == 0 and not list_stack:
                ...
                text, footnote_marks = _inline_to_text_and_marks(inline)
...
def _inline_to_text_and_marks(
    ...
    """``inline`` 토큰을 평문 + 인라인 마크 리스트로 분해한다.

    Returns
    -------
    (text, marks)
        - ``text``: 평문 (footnote 마커는 *제외*; 굵게/기울임 등은 평문화).
        - ``marks``: 각 인라인 마크의 dict 리스트. 현재 footnote 만 다룸.
          항목 형식: ``{"kind": "footnote_ref", "offset": int,
          "footnote_id": int}``. ``offset`` 은 ``text`` 안의 문자 오프셋
          (해당 위치 *직전까지* 누적된 평문 길이; 즉 마커가 들어갈 자리).

    Notes
    -----
    각주 마커 ``[^id]`` 는 결과 평문에 포함되지 않는다. 한/글의
    ``hp:footNote`` 가 자동번호로 마커를 부여하므로 본문에 라벨을 남기지
    않는다 (= A 의 명세 "원문 ID 무시" 와 정합).
    """
