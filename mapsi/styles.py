...
def style_name(style_map: dict[str, Any], role: str, depth: int = 1) -> str:
    ...
    Returns:
        ...
        반환 직전 유니코드 NFC 로 정규화하므로, yaml 이 NFD (자모 분리)
        로 작성돼 있어도 빌더의 ``style_table`` 룩업이 안전하게 매치된다
        (헤더 적재 측도 NFC 정규화하므로 양쪽이 같은 형식으로 만나게 된다).
    ...
    if role in _NESTED_ROLES:
        ...
        return unicodedata.normalize("NFC", str(entry[depth]))
    return unicodedata.normalize("NFC", str(entry))
