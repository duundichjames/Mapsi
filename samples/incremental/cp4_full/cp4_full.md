...
이전: 10_inline
도입 요소:
  - 통합 — 모든 요소가 한 문서 안에 동시 등장
포함 요소:
  - 헤딩 깊이 1 ~ 3
  - 본문 단락 (6708 자, 공백 포함)
  - 번호 리스트 (5 항목), 불릿 리스트 (네모, 2 항목)
  - 인용문 1개 (안에 **bold** + *italic* 포함 — blockquote 인라인 마크
    보존 회귀 방어)
  - 코드 블록 1개 (python fence, 5 줄, "코드" 스타일 한 줄 = 한 단락)
  - 표 2개 (캡션 포함)
  - 그림 2개 (placeholder 이미지 1장 두 번 재활용)
  - 각주 5개
  - 참고문헌 11개 (개요 1 "참고문헌" 헤딩 이후 demote)
  - 수식 인라인 1개 + 디스플레이 1개 (LaTeX 원문 폴백 마커)
  - 인라인 서식 5종 (bold, italic, strikethrough, code, link)
테스트 관점:
  - 단위 골든 (01 ~ 10) 이 잡지 못하는 "요소가 섞일 때만 터지는 회귀" 잡기
  - CP4 합격 기준 (project_plan.md §체크포인트 4) 의 기계 검증 가능 부분 재현
  - 실 학술문서 수준의 분량에서 변환기가 끝까지 도는지 확인
경계 조건 (ADR 0004 정책 준수):
  - 각주 5개와 인라인 수식 1개는 모두 인라인 서식 (bold/italic/strike/code/link)
    이 없는 별개 단락에 배치 — v0.1 에서 한 단락 안 동시 사용 금지
  - 인용문 안의 인라인 서식 (bold + italic) 은 정책 위반이 아님:
    각주/수식과 동시 사용일 때만 금지된다.
사람용 검수:
  - 한컴 오피스에서 열어 문서 흐름이 깨지지 않는지 눈으로 확인
  - 스타일 편집창에서 "본문" 의 글자 크기 변경 시 본문 전체 반영 확인
  - "참고문헌" 섹션 demote 가 끝까지 적용되는지 확인
  - "인용" 스타일 단락이 본문과 시각적으로 구분되는지 확인
  - "코드" 스타일 단락 5줄이 인접 본문과 시각적으로 구분되는지 확인
---
...
이 영역의 후속 도구들은 *Markdown* 발표문의 다음 원칙을 공통으로 계승한다.

> Markdown 으로 작성된 문서는 태그나 서식 지시자가 보이지 않는 **순수 평문 그대로** 출판될 수 있어야 한다. — John Gruber, *Markdown: Syntax* (2004)
...
각 단계는 명시적 함수 경계로 분리되어 있어 단위 테스트가 용이하다. 빌더 단계의 핵심 호출 흐름은 다음과 같다.

```python
def build_paragraph(block: Block, style_map: dict[str, StyleEntry]) -> Element:
    style_id = style_map[styles.style_name(block.role, block.depth)].para_pr_id
    p = etree.Element(f"{HP}p", paraPrIDRef=str(style_id))
    p.append(_make_runs(block.text, block.meta))
    return p
```

style_map 의 키가 한/글 스타일 이름이라는 점이 *진실원 분리* 의 코드 표현이다.
...
![Mapsi 파이프라인 개요](../../../tests/fixtures/sample_figure.png)
...
![두 번째 그림의 자리표시자](../../../tests/fixtures/sample_figure.png)
