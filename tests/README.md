...
| 파일 | 개수 | 단계 | 검증 대상 |
|---|---:|---|---|
| `test_parser.py` | 34 | ① 파서 | 마크다운 → `Block` 리스트 변환 |
| `test_ast_walker.py` | 34 | ② 워크 | 표/그림 캡션 승격 등 문맥 의존 규칙 |
| `test_styles.py` | 28 | ③ 스타일 룩업 | `(role, depth) → 스타일 이름` 매핑 |
| `test_header.py` | 3 | ④ 빌더 (헤더) | `header.xml` 로드 + 스타일표 파싱 |
| `test_elements.py` | 26 | ④ 빌더 (요소) | `Block` → `<hp:p>` / `<hp:tbl>` XML 생성 |
...
| `test_golden.py` | 6 | 끝단 회귀 | `.md` → `.hwpx` 전 파이프라인 |
...
### ② AST walker — `test_ast_walker.py` (34개)

**대상**: `mapsi/ast_walker.py` 의 `walk()` 와
`TABLE_CAPTION_PATTERN` / `FIGURE_CAPTION_PATTERN`.

파서가 만든 평탄 Block 리스트에 문맥 의존 규칙을 적용. 현재는 두
종류의 캡션 승격 규칙 — 표 (직전 단락) / 그림 (직후 단락) — 을
구현. 후속 픽스처에서 참고문헌 / 각주 규칙이 추가되면 케이스가 늘어남.
...
### ④ 빌더 — `test_header.py` + `test_elements.py` (3 + 26 = 29개)
...
- `TestBuildFigureParagraph` (4): 그림 스타일 단락 / alt placeholder /
  빈 alt → `<hp:t>` 생략 / 필수 속성
...
| `06_figure_struct` | 그림 단락 + 캡션 승격 (Phase 6a; 이미지 바이너리 미포함) |
