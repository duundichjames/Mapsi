...
| 파일 | 개수 | 단계 | 검증 대상 |
|---|---:|---|---|
| `test_parser.py` | 41 | ① 파서 | 마크다운 → `Block` 리스트 변환 |
| `test_ast_walker.py` | 41 | ② 워크 | 표/그림 캡션 승격 + 각주 본문 흡수 |
| `test_styles.py` | 28 | ③ 스타일 룩업 | `(role, depth) → 스타일 이름` 매핑 |
| `test_header.py` | 3 | ④ 빌더 (헤더) | `header.xml` 로드 + 스타일표 파싱 |
| `test_elements.py` | 44 | ④ 빌더 (요소) | `Block` → `<hp:p>` / `<hp:tbl>` / `<hp:pic>` / `<hp:footNote>` XML 생성 |
...
| `test_golden.py` | 7 | 끝단 회귀 | `.md` → `.hwpx` 전 파이프라인 |
...
### ② AST walker — `test_ast_walker.py` (41개)
...
### ④ 빌더 — `test_header.py` + `test_elements.py` + `test_bindata.py` + `test_manifest.py` (3 + 44 + 10 + 8 = 65개)
