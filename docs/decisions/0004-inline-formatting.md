...
### 결정 1 — 링크: 라벨 + URL 을 HYPERLINK 필드로 보존 (v0.1.1 갱신)

**v0.1 (초안)**: 라벨만 평문으로 보존, URL 폐기.
**v0.1.1 (현행, 2026-04-19 업데이트)**: 라벨은 평문으로, URL 은 `<hp:fieldBegin>` /
`<hp:fieldEnd>` HYPERLINK 필드의 `name` 속성에 보존한다. 한/글에서 열면
클릭 시 외부 링크가 실제로 브라우저/메일 클라이언트로 연결된다.

최종 emit 구조 (python-hwpx `HwpxOxmlParagraph.add_hyperlink` 와 동일)::
