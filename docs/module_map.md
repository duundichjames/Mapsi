...
│   ├── styles.py          역할(role) + depth → 스타일 이름 조회 순수 함수 (NFC 정규화 방어망)
│   └── inline_styles.py   인라인 마크 조합 → charPrIDRef 룩업 + 디그레이드 정책 (Phase 10)
│
├── 수식 LLM
│   └── math/
...
- `commonmark` 베이스 + `enable("table")` (GFM 표) + `enable("strikethrough")` (`~~취소~~`)
- `mdit_py_plugins.footnote.footnote_plugin` (Pandoc 각주 `[^id]`)
- `mdit_py_plugins.dollarmath.dollarmath_plugin` (LaTeX `$...$` / `$$...$$`)

인라인 토큰 (`strong_open`, `em_open`, `s_open`, `code_inline`, `link_open`) 은
스택 기반으로 추적하여 paragraph 의 `meta["inline_marks"]` 에 `{kind, start, end}`
형태로 보관한다 (ADR 0004). 링크는 라벨 텍스트만 평문에 흡수되고 URL 은 폐기
(v0.2 에서 정식 hyperlink field 마이그레이션 예정).
...
- `build_paragraph` — hp:p (footnote_marks / equation_marks / inline_marks 분기, 셋 중 둘 이상 동시 등장은 v0.1 에서 NotImplementedError)
...
- `_make_runs_with_inline_marks` — 마크 start/end 경계로 텍스트를 분할, `inline_styles.resolve_charpr()` 로 세그먼트별 charPrIDRef 결정 후 인접 동일 charPr 머지 (Phase 10)
...
반환값과 `header.parse_style_table()` 의 키 모두 `unicodedata.normalize("NFC", ...)` 로 정규화된다 (방어망). YAML 이 NFD 로 들어오거나 macOS 외부 도구가 분해형 한글을 흘려도 룩업이 깨지지 않는다.

##### `mapsi/inline_styles.py` (Phase 10)
인라인 마크 집합 → `charPrIDRef` 룩업 테이블 한 개 (`INLINE_CHARPR`) + 디그레이드 로직 (`resolve_charpr`). 사전 등록 조합 5종 (`bold` / `italic` / `bold+italic` / `strike` / `code`) 은 각각 `charPr` 25 ~ 29 로 매핑. 등록 안 된 조합 (예: `bold+italic+strike`) 은 우선순위 (B > I > S > C) 에 따라 가장 약한 마크부터 떨어뜨려 가까운 등록 조합으로 디그레이드한다 (ADR 0004 결정 3).
...
| `docs/decisions/0003-integrate-team-c.md` | Team C 본 구현을 합성 머지 방식으로 인수한 결정 |
| `docs/decisions/0004-inline-formatting.md` | 인라인 서식 5종 (bold/italic/strike/code/link) 의 charPr 사전 등록 정책 |
| `docs/decisions/0005-cp4-integration-golden.md` | CP4 통합 골든 (학술문서 1편 분량) 도입 근거 |
...
- Team C 합성 머지 결정 근거: `docs/decisions/0003-integrate-team-c.md`
- 인라인 서식 정책 (charPr 25 ~ 29 사전 등록): `docs/decisions/0004-inline-formatting.md`
- CP4 통합 골든 (학술문서 1편) 도입 근거: `docs/decisions/0005-cp4-integration-golden.md`
